...
| 2 | Sarah: **"If we can't get results back in about 5 seconds, nobody's going to use it."** | Interactive latency under 5s | Measured p95 **1.50 s** at concurrency 1: `eval/out/report.md` |
| 3 | Sarah: "big importers who dump 200, 300 label applications on us at once" | Batch review | `app/web/static/app.js`; **209 labels/min** with 8 workers |
| 4 | Sarah: "something my mother could figure out... half our team is over 50" | Plain UI, large type, no hunting for buttons | One page, one action, 18px base, artwork beside the checklist, prints cleanly |
...
| 17 | Deliverables: "Deployed Application URL" | A reviewer can open it | Container image; URL in the README |
...
The evaluation reports the split, not just an accuracy figure, and its exit code
gates on harmful outcomes rather than on accuracy. Current result: 19 correct,
3 referred to a human, **0 outcomes that harm anyone**.
...
1. **Calibrate the thresholds on real COLA artwork.** Every number here is tuned
   ...
   sample.
2. **Recover glare.** Adaptive local thresholding or multi-scale retinex would
...
3. **Region-of-interest OCR.** A second targeted pass over the warning block at
   higher resolution would likely close the soft-focus gap.
