...
class EvalSummary:
    ...
    def of_set(self, name: str) -> EvalSummary:
        return EvalSummary(model=self.model,
                           outcomes=[o for o in self.outcomes if o.fixture_set == name])
...
SET_NAMES = {
    "rendered": "Rendered by `fixtures/generate.py`",
    "ai": "Generated by an image model",
}
SET_SHORT = {"rendered": "rendered", "ai": "AI-generated"}


def render_sets(summary: EvalSummary, a) -> None:
    sets = [name for name in SET_NAMES if summary.of_set(name).n]
    if len(sets) < 2:
        return
    a("### By fixture set\n")
    a("The rendered set is drawn by the fixture generator, so every word on it is known. "
      "The AI set is artwork from an image model, as the brief suggests. It holds flat "
      "labels with arched and ornamental lettering, and phone photographs. Its truth was "
      "read off each image by eye. The model got five warnings wrong, and those labels are "
      "defective as printed.\n")
    a("| set | labels | correct | compliant, referred | defective, referred | **harmful** |")
    a("|---|---|---|---|---|---|")
    for name in sets:
        part = summary.of_set(name)
        a(f"| {SET_NAMES[name]} | {part.n} | {sum(o.verdict_correct for o in part.outcomes)} | "
          f"{len(part.cautious_misses)} | {len(part.referred_defects)} | "
          f"**{len(part.unsafe_misses)}** |")
    a("")
...
def render(summary: EvalSummary) -> str:
    ...
    parts = [(name, summary.of_set(name)) for name in SET_NAMES]
    parts = [(name, part) for name, part in parts if part.n]
    if len(parts) > 1:
        # One accuracy over both sets would mix labels tuned on with labels
        # never seen, so each set is stated on its own.
        correct = "; ".join(f"{sum(o.verdict_correct for o in part.outcomes)} of {part.n} "
                            f"{SET_SHORT[name]} correct" for name, part in parts)
        a(f"**{summary.n} fixtures · {correct} · {len(summary.unsafe_misses)} harmful**\n")
    else:
